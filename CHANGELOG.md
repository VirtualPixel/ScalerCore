...
- Grown entities sound the part beyond pitch: volume lifts a touch, reverb mix leans into the room for the echo, and sound falloff scales with the factor so a giant carries across the level while a tiny one goes sneaky-quiet at range. All captured and restored with the rest of the audio state. Items are untouched, their explosion fields already scale.
