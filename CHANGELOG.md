...
### Fixed
- Shrunken monsters stopped floating. Scaling a mesh whose pivot sits above its feet pulls the feet up toward the pivot, so a shrunken enemy hovered off the ground by the pivot-to-feet distance times the shrink. Enemy meshes now measure their pivot-to-floor distance at setup (a raycast on the game's own floor mask, with a renderer-bounds fallback) and hold a proportional stance in both directions; the same lift keeps a capped giant's feet planted instead of sunk. Measuring to the floor rather than to the mesh's lowest point matters for the Robe: her hem hovers by design, and hem-height correction alone left a shrunken Robe carrying her full-size hover gap. The Floater keeps the shrink correction but skips the grow grounding, it hovers on purpose.
